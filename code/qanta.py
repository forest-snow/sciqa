...
if __name__ == '__main__':
    ...
    parser.add_argument('-We', help='location of word embeddings', default='../data/quizbowl/qb_We')
    ...
    for tree in train_trees:
        tree.ans_list = [vocab.index(guess[0]) for guess in tree.guesses \
                            if guess[0] != tree.ans]

    # generate params / We
    params = gen_dtrnn_params(args['d'], rel_list)
    rel_list = list(params[0].keys())
    ...
    for tdata in [train_trees]:
        ...
        for epoch in range(0, args['num_epochs']):
            ...
            batches = [tdata[x : x + args['batch_size']] for x in range(0, len(tdata), 
                       args['batch_size'])]
